...
export const BooksProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Books are PUBLIC content - always try to load from API regardless of auth status
      // The API doesn't require authentication for reading books
  ...
  useEffect(() => {
    ...
    // Books are PUBLIC content - always load from API
    console.log('📚 BooksContext: Initial load - fetching books from API');
    loadData();
  }, []);

  // Also reload when route changes (e.g., after navigation from landing page) - but only once
  useEffect(() => {
